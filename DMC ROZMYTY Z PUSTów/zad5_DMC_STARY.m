...
for nr=1:liczba_regulatorow
    ...
    Nu(nr)=params_DMC{liczba_regulatorow}(nr,3);
end
y=zeros(1, kk);
Y=zeros(1, kk); 
Y(1:12)=Ypp;
...
yzad(round(4*kk/7):round(5*kk/7))=0.6;
yzad(round(5*kk/7):round(6*kk/7))=1.4;
yzad(round(6*kk/7):round(7*kk/7))=1.2;
yzad=yzad-Ypp;
...
title('Sygna³ sterowania DMC'); xlabel('k');ylabel('wartoœæ sygna³u');

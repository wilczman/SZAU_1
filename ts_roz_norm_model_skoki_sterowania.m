...
for i=1:length(delta_u)
    ...
    h2lin_out(:,i) = (V2lin/C2-h2lin^3)/(3*h2lin^2)+h2lin; 
    t=tau+1;
end
hold off
legend
...
for i=1:length(delta_u)
    ...
    h2roz(t)=nthroot(V2roz(t)/C2, 3);
    
    %%%%obliczenie V1 odpowiadajacej V2 rozmytemu%%%%%%%
    V1roz(t)=V1roz(t-1);
...
figure
str=sprintf('Liczba modeli lokalnych=%d',liczba_regulatorow);
title({'Wyjœcie modelu rozmytego',str})
xlabel('t [s]')
ylabel('Wysokoœæ s³upa cieczy h_2 [cm^3/s]')
hold on
for i=1:length(delta_u)
